...
const [res1, res2, res3, res4, res5] = await Promise.all([
  ...
  fetch(`https://restcountries.com/v3.1/all?fields=cca2,demonyms,coatOfArms,car`),
]);
...
if (!res5.ok) throw new Error(`API error (extras): ${res5.status}`);

const [data, areaData, latlngData, bordersData, extrasData] = await Promise.all([res1.json(), res2.json(), res3.json(), res4.json(), res5.json()]);
...
const extrasMap = new Map(extrasData.map(c => [c.cca2, { demonyms: c.demonyms, coatOfArms: c.coatOfArms, car: c.car }]));

const valid = data
  .filter(c => c.flags?.svg && c.name?.common)
  .map(c => ({ ...c, area: areaMap.get(c.cca2) ?? null, latlng: latlngMap.get(c.cca2) ?? null, borders: bordersMap.get(c.cca2) ?? [], ...(extrasMap.get(c.cca2) ?? {}) }))
